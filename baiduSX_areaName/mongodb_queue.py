...
class MongoQueue():
    ...
    def complete_set(self, cardNum, pn):
        # > db.json_url03.find({'_id':{$regex:'1983.*?7\\d{3}'}})
        self.db.update({'_id': {'$regex': cardNum + '.*?' + pn[0] + '\\d{3}'}}, {'$set': {'status': self.WAIT}},
                       multi=True)  # not {'multi': 'true'}
        print(cardNum + '系列MaxPage为' + pn)
    ...
    def set_wait(self, url):
        self.db.update({'_id': url}, {'$set': {'status': self.WAIT}})
